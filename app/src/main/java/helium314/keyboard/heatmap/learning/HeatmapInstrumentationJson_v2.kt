...
// ai-note: Phase1 step 1.3 — JSON export schema v2 with swipe trace + calibration ring
...
object HeatmapInstrumentationJson_v2 {

    const val SCHEMA_VERSION = 3

    fun build(
        context: Context,
        wordMemory: List<WordSession_v4>,
        sessions: List<WordSession_v4>,
    ...
    private fun wordMemoryStatsBlock(): JSONObject = JSONObject().apply {
        put("maxWordEntries", HeatmapWordMemoryLedger_v1.maxWordEntries())
        put("maxCharBudget", HeatmapWordMemoryLedger_v1.maxCharBudget())
        put("entryCount", HeatmapWordMemoryLedger_v1.entryCount())
        put("usedCharBudget", HeatmapWordMemoryLedger_v1.usedCharBudget())
        put("keptCount", HeatmapWordMemoryLedger_v1.keptCount())
        put("erasedCount", HeatmapWordMemoryLedger_v1.erasedCount())
    }
    ...
    private fun buildInfo(): JSONObject = JSONObject().apply {
        ...
        put("swipeTuningRevision", helium314.keyboard.heatmap.swipe.HeatmapSwipeTuningConstants_v2.TUNING_REVISION)
    }
    ...
    private fun swipeRingBlock(): JSONArray = JSONArray().apply {
        HeatmapSwipeTraceRingBuffer_v1.copyEntriesNewestFirst().forEach { e ->
            put(
                JSONObject().apply {
    ...
    private fun sessionToJson(s: WordSession_v4): JSONObject = JSONObject().apply {
