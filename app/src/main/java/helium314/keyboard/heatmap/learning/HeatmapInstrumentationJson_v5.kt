...
// ai-note: v5 (schema 5) — adds POSITION-AWARE precision data on top of v4:
//   per-session: fieldStartOffset, fieldEndOffset, coherence, possibleMindChange, proofreadEdit,
//                supersededBySlotId  (so the offline harness can filter SUSPECT/ORPHAN and only train
//                on coherent strokes)
//   correction chains: now sourced from HeatmapSwipeCorrectionChain_v2 (gated) with finalStartOffset
//                and per-attempt fieldStartOffset
//   NEW top-level "orphanAttempts": erased swipes that did NOT match the kept word at their position
//                (abandoned / mind-change), preserved with full geometry but separated from clean data.
// Mirrors HeatmapInstrumentationJson_v4 for everything else. Used by HeatmapWordSlotSession_v7.
...
object HeatmapInstrumentationJson_v5 {
