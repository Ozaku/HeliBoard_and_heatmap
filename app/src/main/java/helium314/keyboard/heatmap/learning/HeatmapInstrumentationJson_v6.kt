...
// ai-note: v6 (schema 6) — anchor-driven decoder visibility on top of v5. Adds, per session, a
// "swipeAnchors" block surfacing the v2 decoder's ground truth:
//   anchorKeys           = the ordered intended-letter keys the decoder derived from the gesture
//                          (start + velocity-minima stops + corners + end) — this is what the user
//                          asked to verify equals the letters they meant to type
//   anchorCount          = anchorKeys length (the STRICT word length the decoder bound to)
//   strictCandidateCount = how many dictionary words matched strictly (0 => a fallback tier surfaced)
//   decodedWord          = the chosen top word
// Also records gestureDecoderTuningRevision (HeatmapGestureTuningConstants_v2 == 7) in build info.
// Exported geometry cornerPoints now come from HeatmapSwipeGeometryVector_v3 (v2 anchors incl.
// stops); the full raw trace (512-cap) remains the ground truth for tuning anchor thresholds.
// Everything else mirrors HeatmapInstrumentationJson_v5. Used by HeatmapWordSlotSession_v7.
...
object HeatmapInstrumentationJson_v6 {
    ...
    fun build(
        ...
        root.put("orphanAttempts", orphanAttemptsBlock())
    ...
    private fun buildInfo(): JSONObject = JSONObject().apply {
        ...
        put(
            "gestureDecoderTuningRevision",
            helium314.keyboard.heatmap.swipe.HeatmapGestureTuningConstants_v2.TUNING_REVISION,
        )
    }
    ...
    private fun swipeCorrectionChainsBlock(): JSONArray = JSONArray().apply {
        HeatmapSwipeCorrectionChain_v2.copyResolvedOldestFirst().forEach { chain ->
            put(
                JSONObject().apply {
                    put("chainId", chain.chainId)
                    put("finalWord", chain.finalWord)
                    put("finalStartOffset", chain.finalStartOffset)
                    put(
                        "attempts",
                        JSONArray().apply {
                            chain.attempts.forEach { attempt ->
                                put(
                                    JSONObject().apply {
                                        ...
                                        put("fieldStartOffset", attempt.fieldStartOffset)
                                        attempt.swipeVisitOrder?.let { put("swipeVisitOrder", it) }
    ...
    private fun orphanAttemptsBlock(): JSONArray = JSONArray().apply {
        HeatmapSwipeCorrectionChain_v2.copyOrphansOldestFirst().forEach { o ->
            put(
                JSONObject().apply {
                    put("memorySequence", o.memorySequence)
                    put("attemptText", o.attemptText)
                    put("outcome", o.outcome.name)
                    put("fieldStartOffset", o.fieldStartOffset)
                    o.swipeVisitOrder?.let { put("swipeVisitOrder", it) }
                    o.nearestKeptWord?.let { put("nearestKeptWord", it) }
                    o.geometry?.let { put("swipeGeometry", geometryToJson(it)) }
                },
            )
        }
    }
    ...
    private fun sessionToJson(s: WordSession_v5): JSONObject = JSONObject().apply {
        ...
        // ai-note: schema-5 position-aware precision block
        put("fieldStartOffset", s.fieldStartOffset)
        put("fieldEndOffset", s.fieldEndOffset)
        put("coherence", s.coherence.name)
        put("possibleMindChange", s.possibleMindChange)
        put("proofreadEdit", s.proofreadEdit)
        s.supersededBySlotId?.let { put("supersededBySlotId", it.value) }
        ...
        s.swipeDecodeDiagnostics?.let { d ->
            ...
            // ai-note: schema-6 anchor-driven decoder ground truth. With the v2 decoder,
            // diagnostics.visitOrder carries the ANCHOR key sequence (intended letters) and
            // diagnostics.transitKeyCount carries the strict candidate count.
            put(
                "swipeAnchors",
                JSONObject().apply {
                    put("anchorKeys", d.visitOrder)
                    put("anchorCount", d.visitOrder.length)
                    put("strictCandidateCount", d.transitKeyCount)
                    put("decodedWord", d.intentPath)
                    put("tuningRevision", d.tuningRevision)
                },
            )
        }
