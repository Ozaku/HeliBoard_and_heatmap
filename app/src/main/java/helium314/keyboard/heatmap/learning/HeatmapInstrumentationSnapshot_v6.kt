...
// ai-note: v6 — stores snapshot beta label so Settings proves which APK wrote last commit
...
object HeatmapInstrumentationSnapshot_v6 {

    private const val TAG = "HeatmapInstr"

    private const val PREF_SNAPSHOT_BETA = "heatmap_snapshot_beta"
    ...
    fun save(context: Context, summary: LiveSummary, lastSession: WordSession_v5?): Boolean {

        val prefs = HeatmapCrossProcessPrefs_v2.readPrefs(context)

        val lifetime = prefs.getInt(PREF_TOTAL_COMMITS, 0) + 1

        val ok = HeatmapCrossProcessPrefs_v2.editCommit(context) {

            putString(PREF_SNAPSHOT_BETA, HeatmapLearningBuildInfo_v1.BETA_VERSION)
        ...
        if (!ok) Log.w(TAG, "snapshot save failed")
    ...
    fun formatForDebugSummary(context: Context, @Suppress("UNUSED_PARAMETER") liveInThisProcess: LiveSummary): String {
        ...
        return buildString {
            ...
            if (snap.lifetimeCommits <= 0) {
                ...
                append("\nSnapshot written by beta: ").append(snap.snapshotBeta ?: "? (old APK?)")
    ...
    private fun read(prefs: SharedPreferences): Snapshot = Snapshot(
        ...
        snapshotBeta = prefs.getString(PREF_SNAPSHOT_BETA, null),
    ...
    private fun read(context: Context): Snapshot = read(HeatmapCrossProcessPrefs_v2.readPrefs(context))
