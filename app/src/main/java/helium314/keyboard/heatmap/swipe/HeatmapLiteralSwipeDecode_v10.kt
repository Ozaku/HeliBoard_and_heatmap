...
// ai-note: Block 3 step 15i — bridge collapse + reachable pool + dwell doubles
...
object HeatmapLiteralSwipeDecode_v10 {

    private const val TAG = "HeatmapLiteralSwipe"

    data class DecodeResult(
        val ranked: List<SuggestedWordInfo>,
        val infer: HeatmapSwipeSegmentInfer_v7.Result?,
    )
    ...
    private fun decodeInternal(
        ...
        val infer = HeatmapSwipeSegmentInfer_v7.infer(keyboard, pointers)
        ...
        val cacheKey = infer.pathLetters.joinToString("") + "|" + infer.endKeyLabel + "|" + dwell
        ...
        val prefixes = HeatmapSwipePrefixEngine_v8.buildPrefixVariants(infer)
        val scored = HeatmapSwipeCandidateSource_v10.collectDictionaryCandidates(
            prefixes = prefixes,
            infer = infer,
