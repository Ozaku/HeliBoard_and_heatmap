...
// ai-note: Block 3 step 15l — word touch gate + touch sample counts in log
...
object HeatmapLiteralSwipeDecode_v13 {

    private const val TAG = "HeatmapLiteralSwipe"

    data class DecodeResult(
        val ranked: List<SuggestedWordInfo>,
        val infer: HeatmapSwipeSegmentInfer_v10.Result?,
    )
    ...
    private fun decodeInternal(
        ...
        val infer = HeatmapSwipeSegmentInfer_v10.infer(keyboard, pointers)
        ...
        Log.i(
            ...
            "mode=$mode shape=${infer.straightLine.shape} beatsRaw=${infer.beatCountRaw} beats=${infer.beatCount} " +
                ...
                "strokeOrder=[$strokeOrder] touched=[$touched] touchCounts=[$touchCounts] rejected=[$rejected] " +
                "start=${infer.startKeyLabel} end=${infer.endKeyLabel} dwell=[$dwell]",
        )
        val prefixes = HeatmapSwipePrefixEngine_v11.buildPrefixVariants(infer)
        val scored = HeatmapSwipeCandidateSource_v13.collectDictionaryCandidates(
            prefixes = prefixes,
            infer = infer,
        ...
        val ranked = HeatmapSwipeSlotRejectMemory_v4.filterRanked(
