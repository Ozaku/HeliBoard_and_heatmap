...
// ai-note: Block 3 step 15m — full path always; min word len guard; no dwell doubles
...
object HeatmapLiteralSwipeDecode_v14 {

    private const val TAG = "HeatmapLiteralSwipe"

    data class DecodeResult(
        val ranked: List<SuggestedWordInfo>,
        val infer: HeatmapSwipeSegmentInfer_v11.Result?,
    )
    ...
    private fun decodeInternal(
        ...
        val infer = HeatmapSwipeSegmentInfer_v11.infer(keyboard, pointers)
        ...
        Log.i(
            ...
            "mode=$mode shape=${infer.straightLine.shape} beatsRaw=${infer.beatCountRaw} beats=${infer.beatCount} " +
                "maxLen=${infer.maxWordLength} pathRaw=${infer.pathLettersRaw.joinToString("")} " +
        ...
        val prefixes = HeatmapSwipePrefixEngine_v12.buildPrefixVariants(infer)
        val scored = HeatmapSwipeCandidateSource_v14.collectDictionaryCandidates(
            prefixes = prefixes,
            infer = infer,
        ...
        val rejectFiltered = HeatmapSwipeSlotRejectMemory_v5.filterRanked(bumped, infer)
        val ranked = HeatmapSwipeMinWordLenGuard_v1.filterRanked(rejectFiltered, infer)
