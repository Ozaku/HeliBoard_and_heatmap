...
// ai-note: Block 3 step 15o — sensitive beat v2 + wiggle path; logs beatsRaw for E zig-zag debug
...
object HeatmapLiteralSwipeDecode_v16 {

    private const val TAG = "HeatmapLiteralSwipe"

    data class DecodeResult(
        val ranked: List<SuggestedWordInfo>,
        val infer: HeatmapSwipeSegmentInfer_v13.Result?,
    )
    ...
    private fun decodeInternal(
        ...
        val infer = HeatmapSwipeSegmentInfer_v13.infer(keyboard, pointers)
        ...
        val prefixes = HeatmapSwipePrefixEngine_v13.buildPrefixVariants(inferV12)
        val scored = HeatmapSwipeCandidateSource_v16.collectDictionaryCandidates(
            prefixes = prefixes,
            infer = infer,
        ...
        val ranked = HeatmapSwipeMinWordLenGuard_v3.filterRanked(rejectFiltered, infer)
