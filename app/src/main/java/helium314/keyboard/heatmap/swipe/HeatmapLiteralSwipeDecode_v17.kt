...
// ai-note: Block 3 step 15p — full path for long words; scoped lift-end for feet/fee only
...
object HeatmapLiteralSwipeDecode_v17 {

    private const val TAG = "HeatmapLiteralSwipe"

    data class DecodeResult(
        val ranked: List<SuggestedWordInfo>,
        val infer: HeatmapSwipeSegmentInfer_v14.Result?,
    )
    ...
    private fun decodeInternal(
        ...
        val infer = HeatmapSwipeSegmentInfer_v14.infer(keyboard, pointers)
        ...
        val prefixes = HeatmapSwipePrefixEngine_v14.buildPrefixVariants(inferV12)
        val scored = HeatmapSwipeCandidateSource_v17.collectDictionaryCandidates(
            prefixes = prefixes,
            infer = infer,
        ...
        val ranked = HeatmapSwipeMinWordLenGuard_v4.filterRanked(rejectFiltered, infer)
