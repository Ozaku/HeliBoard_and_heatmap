...
// ai-note: Block 3 step 15q — multi-corner path preserved; logs multiCorner= for FLYING debug
...
object HeatmapLiteralSwipeDecode_v18 {

    private const val TAG = "HeatmapLiteralSwipe"

    data class DecodeResult(
        val ranked: List<SuggestedWordInfo>,
        val infer: HeatmapSwipeSegmentInfer_v15.Result?,
    )
    ...
    private fun decodeInternal(
        ...
        val infer = HeatmapSwipeSegmentInfer_v15.infer(keyboard, pointers)
        if (infer == null || infer.pathLetters.isEmpty()) {
            Log.i(TAG, "decode skip — no geometry inference")
            return DecodeResult(ranked = emptyList(), infer = null)
        }
        val inferV12 = HeatmapSwipeInferCompat_v3.toV12(infer)
        ...
        val cacheKey = infer.pathLetters.joinToString("") + "|" + infer.endKeyLabel + "|" + touched +
            "|" + infer.beatCountRaw + "|" + infer.multiCorner
        ...
        Log.i(
            TAG,
            "mode=$mode multiCorner=${infer.multiCorner} shape=${infer.straightLine.shape} " +
                "beatsRaw=${infer.beatCountRaw} beats=${infer.beatCount} maxLen=${infer.maxWordLength} " +
                "requireEnd=$requireEnd pathRaw=${infer.pathLettersRaw.joinToString("")} " +
        ...
        val prefixes = HeatmapSwipePrefixEngine_v15.buildPrefixVariants(inferV12)
        val scored = HeatmapSwipeCandidateSource_v18.collectDictionaryCandidates(
            prefixes = prefixes,
            infer = infer,
        ...
        val ranked = HeatmapSwipeMinWordLenGuard_v5.filterRanked(rejectFiltered, infer)
