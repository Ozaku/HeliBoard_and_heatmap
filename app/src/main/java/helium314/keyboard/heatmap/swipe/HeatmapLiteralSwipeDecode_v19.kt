...
// ai-note: Block 3 step 15r — corner-beat path; logs pathLen= to catch visit-sweep regression
...
object HeatmapLiteralSwipeDecode_v19 {

    private const val TAG = "HeatmapLiteralSwipe"

    data class DecodeResult(
        val ranked: List<SuggestedWordInfo>,
        val infer: HeatmapSwipeSegmentInfer_v16.Result?,
    )
    ...
    private fun decodeInternal(
        ...
        val infer = HeatmapSwipeSegmentInfer_v16.infer(keyboard, pointers)
        if (infer == null || infer.pathLetters.isEmpty()) {
            Log.i(TAG, "decode skip — no geometry inference")
            return DecodeResult(ranked = emptyList(), infer = null)
        }
        val inferV12 = HeatmapSwipeInferCompat_v4.toV12(infer)
        ...
        val cacheKey = infer.pathLetters.joinToString("") + "|" + infer.endKeyLabel + "|" + touched +
            "|" + infer.beatCountRaw + "|" + infer.pathLetters.size
        ...
        Log.i(
            ...
            "mode=$mode pathLen=${infer.pathLetters.size} strokeLen=${infer.strokeOrderLetters.size} " +
                "shape=${infer.straightLine.shape} beatsRaw=${infer.beatCountRaw} beats=${infer.beatCount} " +
        ...
        val prefixes = HeatmapSwipePrefixEngine_v16.buildPrefixVariants(inferV12)
        val scored = HeatmapSwipeCandidateSource_v19.collectDictionaryCandidates(
            prefixes = prefixes,
            infer = infer,
        ...
        val ranked = HeatmapSwipeMinWordLenGuard_v6.filterRanked(rejectFiltered, infer)
