...
// ai-note: Block 3 step 15s — baseline decode; logs doubleHints= for prefix-only doubles
...
object HeatmapLiteralSwipeDecode_v20 {

    private const val TAG = "HeatmapLiteralSwipe"

    data class DecodeResult(
        val ranked: List<SuggestedWordInfo>,
        val infer: HeatmapSwipeSegmentInfer_v17.Result?,
    )
    ...
    private fun decodeInternal(
        ...
        val infer = HeatmapSwipeSegmentInfer_v17.infer(keyboard, pointers)
        if (infer == null || infer.pathLetters.isEmpty()) {
            Log.i(TAG, "decode skip — no geometry inference")
            return DecodeResult(ranked = emptyList(), infer = null)
        }
        val inferV12 = HeatmapSwipeInferCompat_v5.toV12(infer)
        ...
        val cacheKey = infer.pathLetters.joinToString("") + "|" + infer.endKeyLabel + "|" + touched +
            "|" + infer.beatCountRaw + "|" + doubleHints
        ...
        Log.i(
            TAG,
            "mode=$mode pathLen=${infer.pathLetters.size} strokeLen=${infer.strokeOrderLetters.size} " +
                ...
                "strokeOrder=[$strokeOrder] touched=[$touched] touchCounts=[$touchCounts] " +
                "rejected=[$rejected] start=${infer.startKeyLabel} end=${infer.endKeyLabel}",
        )
        val prefixes = HeatmapSwipePrefixEngine_v17.buildPrefixVariants(
            inferV12, infer.doublePrefixIndices,
        )
        val scored = HeatmapSwipeCandidateSource_v20.collectDictionaryCandidates(
            prefixes = prefixes,
            infer = infer,
        ...
        val ranked = HeatmapSwipeMinWordLenGuard_v7.filterRanked(rejectFiltered, infer)
