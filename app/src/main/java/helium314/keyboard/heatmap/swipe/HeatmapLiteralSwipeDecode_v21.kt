...
// ai-note: Block 3 step 15t — start-anchored dict lookup; logs startAnchor in path line
...
object HeatmapLiteralSwipeDecode_v21 {

    private const val TAG = "HeatmapLiteralSwipe"

    data class DecodeResult(
        val ranked: List<SuggestedWordInfo>,
        val infer: HeatmapSwipeSegmentInfer_v18.Result?,
    )
    ...
    private fun decodeInternal(
        ...
        val infer = HeatmapSwipeSegmentInfer_v18.infer(keyboard, pointers)
        ...
        val cacheKey = infer.pathLetters.joinToString("") + "|" + infer.endKeyLabel + "|" + touched +
            "|" + infer.beatCountRaw + "|" + doubleHints + "|" + startAnchor
        ...
        Log.i(
            ...
            "mode=$mode pathLen=${infer.pathLetters.size} strokeLen=${infer.strokeOrderLetters.size} " +
                ...
                "rejected=[$rejected] startAnchor=$startAnchor end=${infer.endKeyLabel}",
        )
        val prefixes = HeatmapSwipePrefixEngine_v18.buildPrefixVariants(
            inferV12, infer.doublePrefixIndices,
        )
        val scored = HeatmapSwipeCandidateSource_v21.collectDictionaryCandidates(
            prefixes = prefixes,
            infer = infer,
        ...
        val ranked = HeatmapSwipeMinWordLenGuard_v8.filterRanked(rejectFiltered, infer)
