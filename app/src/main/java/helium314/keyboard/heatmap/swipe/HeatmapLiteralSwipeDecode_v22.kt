...
// ai-note: Phase1 step 3.3 — full decode pipeline with trace/diagnostics export stash

package helium314.keyboard.heatmap.swipe

import helium314.keyboard.heatmap.learning.HeatmapPathCapture_v1
...
object HeatmapLiteralSwipeDecode_v22 {
    ...
    private fun decodeInternal(
        ...
        val stashPointCount = HeatmapPathCapture_v1.peekPointerSize()
        ...
        val infer = HeatmapSwipeSegmentInfer_v19.infer(keyboard, pointers)
        ...
        val prefixes = HeatmapSwipePrefixEngine_v19.buildPrefixVariants(
            infer, infer.doublePrefixIndices, graph,
        )
        val mode = if (isPreview) "preview" else "full"
        Log.i(
            TAG,
            "mode=$mode tuning=${HeatmapSwipeTuningConstants_v1.TUNING_REVISION} " +
        ...
        val scored = HeatmapSwipeCandidateSource_v22.collectDictionaryCandidates(
            prefixes = prefixes,
            infer = infer,
        ...
        if (scored.isEmpty()) {
            ...
            HeatmapSwipeDecodeSnapshot_v2.stash(infer, emptyList(), diagnostics)
        ...
        val ranked = HeatmapSwipeMinWordLenGuard_v9.filterRanked(rejectFiltered, infer)
        val runnerUps = ranked.map { it.mWord?.toString().orEmpty() }
        HeatmapSwipeDecodeSnapshot_v2.stash(infer, runnerUps, diagnostics)
