...
// ai-note: v24 — intent-primary decode; tuning revision 3; cross-thread snapshot + path capture
...
object HeatmapLiteralSwipeDecode_v24 {
    ...
    private fun decodeInternal(
        ...
        if (infer == null ||
            (infer.pathLetters.isEmpty() && infer.intentPathLetters.isEmpty())
        ) {
        ...
        val prefixes = HeatmapSwipePrefixEngine_v20.buildPrefixVariants(
            infer, infer.doublePrefixIndices, graph,
        )
        val mode = if (isPreview) "preview" else "full"
        Log.i(
            TAG,
            "mode=$mode tuning=${HeatmapSwipeTuningConstants_v3.TUNING_REVISION} " +
        ...
        val scored = HeatmapSwipeCandidateSource_v24.collectDictionaryCandidates(
            prefixes = prefixes,
            infer = infer,
