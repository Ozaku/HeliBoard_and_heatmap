...
// ai-note: Block 3 step 15l — word touch gate + path align; fat/fart impossible without a touched

package helium314.keyboard.heatmap.swipe

object HeatmapLiteralSwipeScore_v10 {

    private const val EXACT_WEIGHT = 1.0
    private const val DOUBLE_LETTER_BOOST = 1.14
    private const val SKIP_PATH_WEIGHT = 0.44
    ...
    fun scoreWord(
        ...
        if (!HeatmapSwipeReachableLetters_v3.isCandidateReachable(
                candidate, pathLetters, dwellDoubleLetters,
            )
        ) {
            return Double.NEGATIVE_INFINITY
        }
        val base = alignScore(lower, pathLetters, dwellDoubleLetters)
