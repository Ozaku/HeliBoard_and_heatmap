...
// ai-note: Block 3 step 15m — no dwell doubles; end match only when path has 2 letters (I'm)

package helium314.keyboard.heatmap.swipe

object HeatmapLiteralSwipeScore_v11 {
    ...
    fun scoreWord(
        ...
        if (!HeatmapSwipeWordTouchGate_v1.isAllowed(candidate, touchedLetters, emptySet())) {
            return Double.NEGATIVE_INFINITY
        }
        ...
        val strictEnd = pathLetters.size == 2
        if (strictEnd && !endLabel.isNullOrEmpty() && lower.length >= 2) {
            if (lower.lastOrNull()?.toString() != endLabel) {
                return Double.NEGATIVE_INFINITY
            }
        }
        if (!HeatmapSwipeReachableLetters_v4.isCandidateReachable(candidate, pathLetters)) {
            return Double.NEGATIVE_INFINITY
        }
        return alignScore(lower, pathLetters)
    }
