...
// ai-note: v15 — dynamic path skips scale with candidate length for longer words

package helium314.keyboard.heatmap.swipe

object HeatmapLiteralSwipeScore_v15 {
    ...
    /** ai-note: allow extra skips when candidate is longer than inferred path */
    private fun maxSkipsFor(candidateLen: Int, pathLen: Int): Int {
        val base = HeatmapSwipeIntentPrototype_v1.maxDictPathSkips
        val extra = (candidateLen - pathLen).coerceAtLeast(0).coerceAtMost(4)
        return (base + extra / 2).coerceAtMost(5)
    }
