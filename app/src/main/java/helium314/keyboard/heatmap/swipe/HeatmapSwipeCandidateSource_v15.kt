...
// ai-note: Block 3 step 15n — lift-end gate + score v12
...
object HeatmapSwipeCandidateSource_v15 {
    ...
    fun collectDictionaryCandidates(
        ...
        val minWordLen = HeatmapSwipeMaxWordLenPolicy_v1.minOutputLen(
            infer.touchedLetters.size,
            infer.pathLetters.size,
            infer.beatCount,
        )

        val pathVariants = HeatmapSwipePathNeighborExpand_v8.expand(infer, maxDictLen)
        ...
        for (prefix in orderedPrefixes) {
            ...
            for (info in results) {
                addScoredCandidate(
                    scored, seen, info, infer, facilitator, minWordLen, maxDictLen, requireEnd,
                )
            }
