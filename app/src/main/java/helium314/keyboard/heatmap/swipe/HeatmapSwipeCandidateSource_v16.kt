...
// ai-note: Block 3 step 15o — min len from v2 policy (wiggle raw beats)
...
object HeatmapSwipeCandidateSource_v16 {
    ...
    fun collectDictionaryCandidates(
        ...
        val inferV12 = HeatmapSwipeInferCompat_v1.toV12(infer)
        ...
        val requireEnd = HeatmapSwipeEndLetterPolicy_v1.requiresEndMatch(inferV12)
        val minWordLen = HeatmapSwipeMaxWordLenPolicy_v2.minOutputLen(
        ...
        val pathVariants = HeatmapSwipePathNeighborExpand_v8.expand(inferV12, maxDictLen)
        val neighborPrefixes = HeatmapSwipePathNeighborExpand_v8.prefixStrings(pathVariants, maxDictLen)
        ...
        for (prefix in orderedPrefixes) {
            ...
            for (info in results) {
                addScoredCandidate(
                    scored, seen, info, inferV12, facilitator, minWordLen, maxDictLen, requireEnd,
                )
            }
    ...
    private fun addScoredCandidate(
        ...
        if (!HeatmapSwipeWordTouchGate_v1.isAllowed(word, infer.touchedLetters, emptySet())) {
            return
        }
        if (requireEnd && !HeatmapSwipeEndLetterPolicy_v1.wordEndsOnLift(word, infer.endKeyLabel)) {
            return
        }
        ...
        val geo = HeatmapLiteralSwipeScore_v12.scoreWord(
            candidate = word,
