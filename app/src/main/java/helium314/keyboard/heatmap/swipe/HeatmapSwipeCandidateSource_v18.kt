...
// ai-note: Block 3 step 15q — long path min-len from v5 guard
...
object HeatmapSwipeCandidateSource_v18 {
    ...
    fun collectDictionaryCandidates(
        ...
        val minWordLen = if (infer.multiCorner && infer.pathLetters.size >= 5) {
            (infer.pathLetters.size - 1).coerceAtLeast(4)
        } else {
            HeatmapSwipeMaxWordLenPolicy_v3.minOutputLen(
        ...
        val pathVariants = HeatmapSwipePathNeighborExpand_v9.expand(inferV12, maxDictLen)
        val neighborPrefixes = HeatmapSwipePathNeighborExpand_v9.prefixStrings(pathVariants, maxDictLen)
