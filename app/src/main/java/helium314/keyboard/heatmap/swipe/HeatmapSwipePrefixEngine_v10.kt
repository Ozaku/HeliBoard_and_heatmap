...
// ai-note: Block 3 step 15k — prefix variants for infer v9; doubles only on touched path letters

package helium314.keyboard.heatmap.swipe

object HeatmapSwipePrefixEngine_v10 {

    private const val MAX_PREFIX_VARIANTS = 20

    fun buildPrefixVariants(infer: HeatmapSwipeSegmentInfer_v9.Result): List<String> {
        ...
        if (!strictTwoKey) {
            ...
            infer.startKeyLabel?.let { if (it.isNotEmpty() && it in infer.touchedLetters) variants.add(it) }
        } else {
        ...
        return variants
            .filter { variant ->
                variant.isNotEmpty() &&
                    ...
                    variant.all { ch -> ch.lowercaseChar().toString() in infer.touchedLetters }
            }
