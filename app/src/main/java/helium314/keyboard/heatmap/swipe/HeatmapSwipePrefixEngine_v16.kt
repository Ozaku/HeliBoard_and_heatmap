...
// ai-note: Block 3 step 15r — v14 end policy + v2 dict doubles; no progressive garbage prefixes

package helium314.keyboard.heatmap.swipe

object HeatmapSwipePrefixEngine_v16 {
    ...
    fun buildPrefixVariants(infer: HeatmapSwipeSegmentInfer_v12.Result): List<String> {
        ...
        val hasWiggle = letters.zipWithNext().any { (a, b) -> a == b }
        val variants = LinkedHashSet<String>()
        if (pathJoin.isNotEmpty()) variants.add(pathJoin)
        addCoreVariants(variants, letters, maxPrefixLen, requireEnd, endLabel)
        for (dictDouble in HeatmapSwipeDictDoublePrefix_v2.prefixVariants(
                letters, maxPrefixLen, hasWiggle,
            )
        ) {
    ...
    private fun addCoreVariants(
        ...
        if (letters.size >= 3 && letters.size <= 8 && maxLen >= 3 && !requireEnd) {
