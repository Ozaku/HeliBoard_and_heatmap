...
// ai-note: Block 3 step 15p — touch v3 + normalize v8; full stroke order path for long words
...
object HeatmapSwipeSegmentInfer_v14 {
    ...
    fun infer(keyboard: Keyboard, pointers: InputPointers): Result? {
        ...
        val raw = HeatmapSwipeRawBeatInfer_v1.infer(keyboard, pointers) ?: return null
        val graph = HeatmapKeyNeighborGraph_v2.fromLayout(layout)
        val touch = HeatmapSwipeStrokeTouchSet_v3.collect(layout, pointers)
        val normalized = HeatmapPathLettersNormalize_v8.normalize(
            raw = raw.pathLetters,
            neighborGraph = graph,
            layout = layout,
            pointers = pointers,
