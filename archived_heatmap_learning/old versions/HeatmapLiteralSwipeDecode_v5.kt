...
// ai-note: Block 3 step 15d — straight-line letter cap; logs shape= for debug
...
object HeatmapLiteralSwipeDecode_v5 {
    ...
    private fun decodeInternal(
        ...
        val infer = HeatmapSwipeSegmentInfer_v4.infer(keyboard, pointers)
        ...
        val prefixes = HeatmapSwipePrefixEngine_v4.buildPrefixVariants(infer)
        val scored = HeatmapSwipeCandidateSource_v5.collectDictionaryCandidates(
            prefixes = prefixes,
            infer = infer,
        ...
        if (ranked.isNotEmpty()) {
            Log.i(TAG, "top=${ranked.first().mWord} runners=${runnerUps.take(4)}")
        }
