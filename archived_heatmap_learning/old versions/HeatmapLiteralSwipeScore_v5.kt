...
// ai-note: Block 3 step 15f — final letter must match lift end; middle letters use path subsequence

package helium314.keyboard.heatmap.swipe

object HeatmapLiteralSwipeScore_v5 {
    ...
    fun scoreWord(
        ...
        for (i in lower.indices) {
            ...
            val (idx, matched) = when (i) {
                ...
                len - 1 -> {
                    if (!endLabel.isNullOrEmpty()) {
                        if (ch == endLabel) len - 1 to true else -1 to false
                    } else {
                        resolveMatch(lower, i, ch, pathLetters, pathIdx)
                    }
                }
