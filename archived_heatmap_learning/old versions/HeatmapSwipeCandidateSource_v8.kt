...
// ai-note: Block 3 step 15g — neighbor path expand + order-first scoring (geo 0.86 / dict 0.14)
...
object HeatmapSwipeCandidateSource_v8 {

    private const val SESSION_ID_TYPING = 0
    private const val GEO_WEIGHT = 0.86
    private const val DICT_WEIGHT = 0.14
    ...
    fun collectDictionaryCandidates(
        ...
        infer: HeatmapSwipeSegmentInfer_v5.Result,
        facilitator: DictionaryFacilitator,
        ngramContext: NgramContext,
        keyboard: Keyboard,
        settings: SettingsValuesForSuggestion,
        ...
        val neighborGraph = layout?.let { HeatmapKeyNeighborGraph_v1.fromLayout(it) }
        ...
        val pathVariants = HeatmapSwipePathNeighborExpand_v1.expand(
            infer.pathLetters, infer.pathLettersRaw, layout,
        )
        val neighborPrefixes = HeatmapSwipePathNeighborExpand_v1.prefixStrings(pathVariants, maxDictLen)
        ...
        for (prefix in orderedPrefixes) {
            ...
            for (info in results) {
                addScoredCandidate(
                    ...
                    strictEnd, minWordLen, maxDictLen,
                )
            }
            if (scored.size >= maxResults * 2) break
        }

        if (strictTwoKey && pathJoin.length >= 2) {
            addSynthetic(scored, seen, pathJoin, pathJoin, LITERAL_PATH_BOOST, infer, neighborGraph, strictEnd)
            for (contraction in HeatmapSwipeContractionExpand_v1.expansions(pathJoin)) {
                if (contraction.length <= maxDisplayLen) {
                    addSynthetic(scored, seen, contraction, pathJoin, CONTRACTION_BOOST, infer, neighborGraph, strictEnd)
                }
            }
        }
    ...
    private fun resolveMaxDictLen(infer: HeatmapSwipeSegmentInfer_v5.Result, strictTwoKey: Boolean): Int {
    ...
    private fun addScoredCandidate(
        ...
        infer: HeatmapSwipeSegmentInfer_v5.Result,
        facilitator: DictionaryFacilitator,
        neighborGraph: HeatmapKeyNeighborGraph_v1.Graph,
        strictEnd: Boolean,
        minWordLen: Int,
        maxDictLen: Int,
    ) {
        ...
        val geo = HeatmapLiteralSwipeScore_v6.scoreWord(
            candidate = word,
    ...
    private fun lengthMultiplier(
        ...
        if (delta == 1 && geo >= 0.75) return base.coerceAtLeast(0.92)
        if (delta == 2 && geo >= 0.82) return base.coerceAtLeast(0.85)
        return base
    }
