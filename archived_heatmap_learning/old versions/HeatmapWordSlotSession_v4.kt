...
// ai-note: v4 — host change reset + step 1.11 correction flush hooks
...
object HeatmapWordSlotSession_v4 {
    ...
    private val wordSessions = WordSessionRegistry_v4()
    ...
    private val lastWordSession = AtomicReference<WordSession_v4?>(null)
    ...
    private fun persistExportSnapshot(context: Context, source: String, archiveOnFlush: Boolean) {
        ...
        val json = HeatmapInstrumentationJson_v2.build(

            context,

            wordMemory = HeatmapWordMemoryLedger_v1.copyOldestFirst(),
    ...
    private fun resetSessionInternal(context: Context) {
        ...
        HeatmapWordMemoryLedger_v1.clear()
    ...
    fun onWordCommitted(
        ...
        val decodeSnap = if (isGestureInput) {
            helium314.keyboard.heatmap.swipe.HeatmapSwipeDecodeSnapshot_v2.consume()
        } else {
            null
        }

        val session = HeatmapWordSessionBuilder_v1.fromDecodeSnapshot(
            slotId = slot,
            ...
            memorySequence = HeatmapWordMemoryLedger_v1.nextMemorySequence(),
            linkedSlotId = null,
            snap = decodeSnap,
        )
        ...
        Log.i(
            ...
            "commit ${session.debugOneLine()} route=$acRoute ime=${context.packageName} " +
                ...
                "memory=${HeatmapWordMemoryLedger_v1.entryCount()}/${HeatmapWordMemoryLedger_v1.maxWordEntries()} " +
                "kept=${HeatmapWordMemoryLedger_v1.keptCount()} erased=${HeatmapWordMemoryLedger_v1.erasedCount()}",
    ...
    fun lastWordSession(): WordSession_v4? = lastWordSession.get()
    ...
    private fun recordWordMemoryAttempt(
        context: Context,
        session: WordSession_v4,
        archiveOnCommit: Boolean,
    ) {
        wordSessions.record(session)
        HeatmapWordMemoryLedger_v1.record(session)
        if (session.inputMode == WordSessionInputMode_v1.SWIPE) {
            HeatmapSwipeTraceRingBuffer_v1.record(session)
        }
    ...
    fun onBatchComposingRejected(
        ...
        val snap = helium314.keyboard.heatmap.swipe.HeatmapSwipeDecodeSnapshot_v2.peek()
        ...
        helium314.keyboard.heatmap.swipe.HeatmapSwipeSlotRejectMemory_v2
